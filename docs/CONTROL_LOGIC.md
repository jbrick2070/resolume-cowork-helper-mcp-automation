...
This is the authoritative ruleset for **performance presets** generated by the
layout prompts (`prompts/03-apc40-layout.md`,
...
synthesis and selected-clip serializer study, then verified against a real
Avenue 7.27 rig. The review transcripts were intentionally removed from the
public release; this file is the retained operating law.

A diagnostic visual twin is a different product. It may deliberately fan one
physical CC into wake, opacity, and motion records while keeping every shortcut
ID and `(raw_key, role, input_path)` tuple unique. Start with
`docs/CONTROLLER_VISUAL_TWIN_PLAYBOOK.md` and use
`docs/ANY_CONTROLLER_VISUAL_TWIN_PROMPT.md` for that workflow. Do not apply its
fan-out to a performance action unless the performance spec explicitly requires
and validates it.
...
5. **Validate every performance preset:** shortcut count = unique ids = unique
   raw MIDI keys, except serializer-required feedback siblings or another
   explicitly documented performance grouping; MIDI output OFF on select
   clones; no CC in both tiers. UTF-8 no BOM. Visual-twin diagnostics use the
   separate declared fan-out contract in the playbook.
