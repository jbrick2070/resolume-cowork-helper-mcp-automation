...
static CFFGLPluginInfo PluginInfo(
	PluginFactory< FableScanline >,           // Create method
	"FSCN",                                   // Plugin unique ID of maximum length 4.
	"Fable Scanline",                         // Plugin name
	...
	lines( 0.25f ), displace( 0.35f ), thickness( 0.5f ), glow( 0.4f ), mixAmount( 1.0f )
{
	...
	FFGLLog::LogToHost( "Created Fable Scanline effect" );
}
FableScanline::~FableScanline()
{
}

FFResult FableScanline::InitGL( const FFGLViewportStruct* vp )
{
...
FFResult FableScanline::ProcessOpenGL( ProcessOpenGLStruct* pGL )
{
...
FFResult FableScanline::DeInitGL()
{
...
FFResult FableScanline::SetFloatParameter( unsigned int dwIndex, float value )
{
	switch( dwIndex )
	{
	case PT_LINES:
...
float FableScanline::GetFloatParameter( unsigned int index )
{
	switch( index )
	{
	case PT_LINES:
