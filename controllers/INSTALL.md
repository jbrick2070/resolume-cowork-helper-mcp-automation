...
## A. Install the shipped preset XML

1. Close Resolume.
2. Copy `APC 40 MK II - Orbit v1.xml` into Resolume's MIDI shortcuts
   folder:
...
5. In the same MIDI preferences, select the preset
   "APC 40 MK II - Orbit v1" for the device.
6. The grid lights within a couple of seconds. Consult `LAYOUT_CARD.md`
   for what every control does.

VERIFY: bottom-left pad launches the bottom-left cell (layer 1, column 1).
If a pad launches the wrong cell, see troubleshooting below.

## B. Map it yourself (the manual road)
...
you want bound to it; repeat. Done > Save. Use `LAYOUT_CARD.md` as your
spec sheet:
